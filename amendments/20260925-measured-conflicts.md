...
**Own-node eligibility fix in flight:** [Math- #10](https://github.com/d6g8k5htny-coder/Math-/pull/10) / [#11](https://github.com/d6g8k5htny-coder/Math-/pull/11) (`chatgpt/pr8-own-node-eligibility-fix`, tip `3dc2f30`) require `CONTROLLING_ELIGIBLE={PROVED_REVIEWED}` on the node itself before `apply_promotion`. Local replay: 28/28 tests OK; author-side+terminal-deps → `REFUSED`; self-`PROVED_REVIEWED`+terminal-deps → `CONTROLLING`. Main #92 `db0645f` handoff notes that patch. Do not race those Math- files from governance-.
